  ...
  return (
    <section id='home' className='w-full p-2 flex xl:flex-row flex-col justify-center min-h-screen gap-10  max-container'>
      <div className='relative xl:w-2/5 flex flex-col justify-center items-start w-full max-xl:padding-x pt-28'>
        ...
        {/* <h1 className='mt-10 font-palanquin  max-sm:text-[72px] text-8xl max-sm:leading-[82] font-bold '>
        <span className='xl:bg-white xl:whitespace-nowrap relative z-10 pr-10'>
        The New Arrival</span> 
        <br />
        <span  className='text-coral-red inline-block mt-3'>Nike</span> Shoes</h1> */}
        <p className='font-montserrat text-slate-gray text-lg leading-8 mt-6 mb-14 sm:max-w-sm'>Discover stylish Nike quality comfort, and innovation for your active life</p>
        ...
        <div className='flex justify-start items-start flex-wrap w-full mt-20 gap-16 '>
          {statistics.map((item) => (
            <div key={item.label}>
              ...
              <p className='leadding-7 font-montserrat text-slate-gray'>{item.label}</p>
            </div>
          ))}
        </div>
      </div>
          ...
          {shoes.map((shoe) => (
            <div key={shoe}>
              <ShoeCard imgURL={shoe}
                ...
                bigShoeImg={bigShoeImg} className="z-20 cursor-pointer"
              />
